...
last_modified_at: 2025-05-10 21:05:11 +0800
...
t&=(\frac{R}{2}+2MG)\left(\frac{R}{2MG}-1\right)^{1/2}\eta+\frac{R}{2}\left(\frac{R}{2MG}-1\right)^{1/2}sin\eta\\&+2MGlog\left|\frac{\left(\frac{R}{2MG}-1\right)^{1/2}+tan\frac{\eta}{2}}{\left(\frac{R}{2MG}-1\right)^{1/2}-tan\frac{\eta}{2}}\right| ~~~[0<\eta<\pi]
\end{align}$$
...
其中$$U,V$$是两个积分常数。
画出史瓦西坐标下的光锥：!*广义相对论引论2ed（俞允强）（北大出版社1997）, p.5*
首先容易发现的一点是$$r>2GM$$区域的光是到不了视界的。
我们主要研究$$r<2GM$$的区域。在该区域中物理的运动规律
...
$$ \begin{align}
&t-r-4GM\ln\left|1-\frac{r}{2GM}\right|=U. \\
&t+r=V
\end{align} $$
...
在$$U,V$$坐标下的图例如图所示：![](https://i.imgur.com/DWSrdnk.png)

$$r$$为常数的曲线是**类时双曲线**（ $$UV=C;C<0$$ )，当$$r\to{2}MG$$时双曲线有两个渐近线$$H^{+}$$和$$H^{-}$$。在克鲁斯卡坐标中视界位于有限处，但是它们依旧位于史瓦西时间$$\pm \infty$$处，于是我们能看到粒子在有限固有时穿越视界，而同时史瓦西时间流逝了无穷多。!*AN INTRODUCTION TO BLACKHOLES,INFORMATION,AND THESTRINGTHEORY REVOLUTION(Susskind & Lindesay), p.9*
和前面的坐标不同，克鲁斯卡度规可以描述$$r<2MG$$的区域，这个区域可以被取为II，II中等$$r$$的线是**类空双曲线**（$$UV=C;C>0$$）。
在$$r=0$$处真正的奇点对应着$$R^{2}=-(MG)^{2}$$，也就是

$$UV=\left(MG\right)^2$$

整个史瓦西几何的最大解析延拓在Kruskal–Szekere坐标下的描述如图：!*AN INTRODUCTION TO BLACKHOLES,INFORMATION,AND THESTRINGTHEORY REVOLUTION(Susskind & Lindesay), p.10*![](https://i.imgur.com/QnpvAIx.png)
