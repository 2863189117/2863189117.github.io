...
令$$\rho\sigma\eta\xi$$取参考顺序的值；于是 (4.4.8) 恰好是$$|\partial x^\prime/\partial x|$$的行列式：

$$\frac{\partial x^{\prime\rho}}{\partial x^\mu}\frac{\partial x^{\prime\sigma}}{\partial x^\nu}\frac{\partial x^{\prime\eta}}{\partial x^\lambda}\frac{\partial x^{\prime\xi}}{\partial x^\kappa}\varepsilon^{\mu\nu\lambda\kappa}=\left|\frac{\partial x^{\prime}}{\partial x}\right|\varepsilon^{\rho\sigma\eta\xi}$$
...
所以$$\varepsilon^{\rho\sigma\eta\xi}$$是$$W=-1$$的张量密度。
权在张量代数中有如下继承关系：
> [!PDF|] *引力和宇宙学——广义相对论的原理和应用 (斯蒂芬·温伯格 (Steven Weinberg)) .pdf, p.95*
