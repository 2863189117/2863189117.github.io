...
$$g^{\prime}=\left\lvert \frac{\partial x}{\partial x^{\prime}}\right\rvert ^2g$$

相比正常的标量，这个张量密度的变换多了**2**个Jacobi行列式因子，我们定义：一个量，其变换比张量的多出几个 Jacobi 行列式因子的，称为张量密度.行列式$$\lvert \partial x^\prime/\partial x\rvert $$因子的数目称为密度的权；例如$$g$$ 是一个权为 -2 的密度。

$$\left\lvert \frac{\partial x}{\partial x^{\prime}}\right\rvert =\left\lvert \frac{\partial x^{\prime}}{\partial x}\right\rvert ^{-1}$$
...
令$$\rho\sigma\eta\xi$$取参考顺序的值；于是 (4.4.8) 恰好是$$\lvert \partial x^\prime/\partial x\rvert $$的行列式：

$$\frac{\partial x^{\prime\rho}}{\partial x^\mu}\frac{\partial x^{\prime\sigma}}{\partial x^\nu}\frac{\partial x^{\prime\eta}}{\partial x^\lambda}\frac{\partial x^{\prime\xi}}{\partial x^\kappa}\varepsilon^{\mu\nu\lambda\kappa}=\left\lvert \frac{\partial x^{\prime}}{\partial x}\right\rvert \varepsilon^{\rho\sigma\eta\xi}$$
