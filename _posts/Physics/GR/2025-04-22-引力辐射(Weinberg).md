...
其中$$\left| h_{\mu \nu}\right|  \ll_{1}$$，近似到$$h$$的一阶，Ricci 张量变为：
...
$$h_{\mu\nu}(\boldsymbol{x},t)=4G\int\mathrm{d}^3\boldsymbol{x}^{\prime}\frac{S_{\mu\nu}(\boldsymbol{x}^{\prime},t-\left| \boldsymbol{x}-\boldsymbol{x}^{\prime}\right|  )}{\left| x-\boldsymbol{x}^{\prime}\right|  }$$
...
$$\begin{aligned}R_{\mu\kappa}^{(2)}&=-\frac{1}{2}h^{\lambda\nu}\left[\frac{\partial^{2}h_{\lambda\nu}}{\partial x^{\kappa}\partial x^{\mu}}-\frac{\partial^{2}h_{\mu\nu}}{\partial x^{\kappa}\partial x^{\lambda}}-\frac{\partial^{2}h_{\lambda\kappa}}{\partial x^{\nu}\partial x^{\mu}}+\frac{\partial^{2}h_{\mu\kappa}}{\partial x^{\nu}\partial x^{\lambda}}\right]\\&+\frac{1}{4}\left[2\frac{\partial h^{\nu}{}_{\sigma}}{\partial x^{\nu}}-\frac{\partial h^{\nu}{}_{\nu}}{\partial x^{\sigma}}\right]\left[\frac{\partial h^{\sigma}{}_{\mu}}{\partial x^{\kappa}}+\frac{\partial h^{\sigma}{}_{\kappa}}{\partial x^{\mu}}-\frac{\partial h_{\mu\kappa}}{\partial x_{\sigma}}\right]\\&-\frac{1}{4}\left[\frac{\partial h_{\sigma\kappa}}{\partial x^{\lambda}}+\frac{\partial h_{\sigma\lambda}}{\partial x^{\kappa}}-\frac{\partial h_{\lambda\kappa}}{\partial x^{\sigma}}\right]\left[\frac{\partial h^{\sigma}{}_{\mu}}{\partial x_{\lambda}}+\frac{\partial h^{\sigma\lambda}}{\partial x^{\mu}}-\frac{\partial h^{\lambda}{}_{\mu}}{\partial x_{\sigma}}\right]\end{aligned}$$
 ...
 里面爆算（此式在*爱因斯坦场方程* 一节中亦有记载），鉴于结果过于复杂，我们只考虑一个很大的空间区域，即 $$\gg\left| \mathbf{k}\right|  ^{-1}$$ 的区域中的能动量平均值 $$t_{\mu \nu}$$，这样就可以消去所有正比于 $$\exp (\pm2\mathrm{i}k_{\lambda}x^{\lambda})$$ 的项，剩下：
...
$$\langle R_{\mu\nu}^{(2)}\rangle=\frac{k_{\mu}k_{\nu}}{2}\left(e^{\lambda\rho*}e_{\lambda\rho}-\frac{1}{2}\left| e^{\lambda}{}_{\lambda}\right|  ^2\right)$$
...
$$\langle t_{\mu\nu}\rangle=\frac{k_{\mu}k_{\nu}}{16\pi G}\left(e^{\lambda\rho*}e_{\lambda\rho}-\frac{1}{2}\left| e^{\lambda}{}_{\lambda}\right|  ^{2}\right)$$

注意前面的*规范（旋转）变换* 将 $$\langle t_{\mu\nu}\rangle$$ 中的诸项变为：

$$\begin{aligned}&e^{\prime\lambda\rho*}e_{\lambda\rho}^{\prime}=\dot{e}^{\lambda\rho*}e_{\lambda\rho}+2\mathrm{Re}\varepsilon_{\rho}^{*}k^{\rho}e^{\lambda}{}_{\lambda}+2\left| \varepsilon_{\rho}k^{\rho}\right|  ^{2}\\&e^{\prime\lambda}{}_{\lambda}=e^{\lambda}{}_{\lambda}+2k^{\lambda}\varepsilon_{\lambda}\end{aligned}$$

但是 $$\langle t_{\mu\nu}\rangle$$ 自己是规范不变的，所以就能动量而言，极化 $$e_{\mu \nu}$$ 和 $$e_{\mu \nu}+k_{\mu}\varepsilon_{\nu}+k_{\nu}\varepsilon_{\mu}$$ 代表同样的物理波，于是我们再一次看到，有物理意义的极化产量只有两个而不是六个。
特别地，一个沿着 $$+z$$ 传播的波具有能动量张量：

$$\langle t_{\mu\nu}\rangle=\frac{k_{\mu}k_{\nu}}{8\pi G}(\left| e_{11}\right|  ^2+\left| e_{12}\right|  ^2)$$

 使用螺旋度的振幅可以写成：

$$\langle t_{\mu\nu}\rangle=\frac{k_\mu k_\nu}{16\pi G}(\left| e_+\right|  ^2+\left| e_-\right|  ^2)$$
...
$$\begin{aligned}h_{\mu\nu}(\boldsymbol{x},t)&=4G\int\frac{\mathrm{d}^{3}x^{\prime}}{\left| x-x^{\prime}\right|  }S_{\mu\nu}(x^{\prime},\omega)\\&\times\exp\{-\mathrm{i}\omega t+\mathrm{i}\omega\left| x-x^{\prime}\right|  \}+\mathrm{c.c.}\end{aligned}$$

其中：

$$S_{\mu\nu}(\boldsymbol{x},\omega)\equiv T_{\mu\nu}(\boldsymbol{x},\omega)-\frac{1}{2}\eta_{\mu\nu}T^\lambda{}_\lambda(\boldsymbol{x},\omega)$$

假定在波区（$$r\equiv\left| \boldsymbol{x}\right|  $$ 远大于源的尺度，远大于 $$\omega R^{2}, \frac{1}{\omega}$$）中观察这个辐射，则分母可以换成 $$r$$，指数上可以做近似：

$$\left| \boldsymbol{x}-\boldsymbol{x}^{\prime}\right|  \simeq r-\boldsymbol{x}^{\prime}\cdot\hat{\boldsymbol{x}},~~~~ \hat{\boldsymbol{x}}\equiv\frac{x}{r}$$
