...
$$h_{\mu\nu}(\boldsymbol{x},t)=4G\int\mathrm{d}^3\boldsymbol{x}^{\prime}\frac{S_{\mu\nu}(\boldsymbol{x}^{\prime},t-|\boldsymbol{x}-\boldsymbol{x}^{\prime}|)}{|x-\boldsymbol{x}^{\prime}|}$$
...
$$\langle R_{\mu\nu}^{(2)}\rangle=\frac{k_{\mu}k_{\nu}}{2}\left(e^{\lambda\rho*}e_{\lambda\rho}-\frac{1}{2}|e^{\lambda}{}_{\lambda}|^2\right)$$
...
$$\langle t_{\mu\nu}\rangle=\frac{k_{\mu}k_{\nu}}{16\pi G}\left(e^{\lambda\rho*}e_{\lambda\rho}-\frac{1}{2}|e^{\lambda}{}_{\lambda}|^{2}\right)$$

注意前面的*规范（旋转）变换* 将 $$\langle t_{\mu\nu}\rangle$$ 中的诸项变为：

$$\begin{aligned}&e^{\prime\lambda\rho*}e_{\lambda\rho}^{\prime}=\dot{e}^{\lambda\rho*}e_{\lambda\rho}+2\mathrm{Re}\varepsilon_{\rho}^{*}k^{\rho}e^{\lambda}{}_{\lambda}+2|\varepsilon_{\rho}k^{\rho}|^{2}\\&e^{\prime\lambda}{}_{\lambda}=e^{\lambda}{}_{\lambda}+2k^{\lambda}\varepsilon_{\lambda}\end{aligned}$$

但是 $$\langle t_{\mu\nu}\rangle$$ 自己是规范不变的，所以就能动量而言，极化 $$e_{\mu \nu}$$ 和 $$e_{\mu \nu}+k_{\mu}\varepsilon_{\nu}+k_{\nu}\varepsilon_{\mu}$$ 代表同样的物理波，于是我们再一次看到，有物理意义的极化产量只有两个而不是六个。
特别地，一个沿着 $$+z$$ 传播的波具有能动量张量：

$$\langle t_{\mu\nu}\rangle=\frac{k_{\mu}k_{\nu}}{8\pi G}(|e_{11}|^2+|e_{12}|^2)$$
...
$$\begin{aligned}h_{\mu\nu}(\boldsymbol{x},t)&=4G\int\frac{\mathrm{d}^{3}x^{\prime}}{|x-x^{\prime}|}S_{\mu\nu}(x^{\prime},\omega)\\&\times\exp\{-\mathrm{i}\omega t+\mathrm{i}\omega|x-x^{\prime}|\}+\mathrm{c.c.}\end{aligned}$$
...
$$|\boldsymbol{x}-\boldsymbol{x}^{\prime}|\simeq r-\boldsymbol{x}^{\prime}\cdot\hat{\boldsymbol{x}},~~~~ \hat{\boldsymbol{x}}\equiv\frac{x}{r}$$
