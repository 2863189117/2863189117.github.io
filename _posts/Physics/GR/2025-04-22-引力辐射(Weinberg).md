...
$$h_{\mu\nu}(\boldsymbol{x},t)=4G\int\mathrm{d}^3\boldsymbol{x}^{\prime}\frac{S_{\mu\nu}(\boldsymbol{x}^{\prime},t-\lvert \boldsymbol{x}-\boldsymbol{x}^{\prime}\rvert )}{\lvert x-\boldsymbol{x}^{\prime}\rvert }$$
...
$$\langle R_{\mu\nu}^{(2)}\rangle=\frac{k_{\mu}k_{\nu}}{2}\left(e^{\lambda\rho*}e_{\lambda\rho}-\frac{1}{2}\lvert e^{\lambda}{}_{\lambda}\rvert ^2\right)$$
...
$$\langle t_{\mu\nu}\rangle=\frac{k_{\mu}k_{\nu}}{16\pi G}\left(e^{\lambda\rho*}e_{\lambda\rho}-\frac{1}{2}\lvert e^{\lambda}{}_{\lambda}\rvert ^{2}\right)$$

注意前面的*规范（旋转）变换* 将 $$\langle t_{\mu\nu}\rangle$$ 中的诸项变为：

$$\begin{aligned}&e^{\prime\lambda\rho*}e_{\lambda\rho}^{\prime}=\dot{e}^{\lambda\rho*}e_{\lambda\rho}+2\mathrm{Re}\varepsilon_{\rho}^{*}k^{\rho}e^{\lambda}{}_{\lambda}+2\lvert \varepsilon_{\rho}k^{\rho}\rvert ^{2}\\&e^{\prime\lambda}{}_{\lambda}=e^{\lambda}{}_{\lambda}+2k^{\lambda}\varepsilon_{\lambda}\end{aligned}$$

但是 $$\langle t_{\mu\nu}\rangle$$ 自己是规范不变的，所以就能动量而言，极化 $$e_{\mu \nu}$$ 和 $$e_{\mu \nu}+k_{\mu}\varepsilon_{\nu}+k_{\nu}\varepsilon_{\mu}$$ 代表同样的物理波，于是我们再一次看到，有物理意义的极化产量只有两个而不是六个。
特别地，一个沿着 $$+z$$ 传播的波具有能动量张量：

$$\langle t_{\mu\nu}\rangle=\frac{k_{\mu}k_{\nu}}{8\pi G}(\lvert e_{11}\rvert ^2+\lvert e_{12}\rvert ^2)$$

 使用螺旋度的振幅可以写成：

$$\langle t_{\mu\nu}\rangle=\frac{k_\mu k_\nu}{16\pi G}(\lvert e_+\rvert ^2+\lvert e_-\rvert ^2)$$
...
$$\begin{aligned}h_{\mu\nu}(\boldsymbol{x},t)&=4G\int\frac{\mathrm{d}^{3}x^{\prime}}{\lvert x-x^{\prime}\rvert }S_{\mu\nu}(x^{\prime},\omega)\\&\times\exp\{-\mathrm{i}\omega t+\mathrm{i}\omega\lvert x-x^{\prime}\rvert \}+\mathrm{c.c.}\end{aligned}$$

其中：

$$S_{\mu\nu}(\boldsymbol{x},\omega)\equiv T_{\mu\nu}(\boldsymbol{x},\omega)-\frac{1}{2}\eta_{\mu\nu}T^\lambda{}_\lambda(\boldsymbol{x},\omega)$$

假定在波区（$$r\equiv\lvert \boldsymbol{x}\rvert $$ 远大于源的尺度，远大于 $$\omega R^{2}, \frac{1}{\omega}$$）中观察这个辐射，则分母可以换成 $$r$$，指数上可以做近似：

$$\lvert \boldsymbol{x}-\boldsymbol{x}^{\prime}\rvert \simeq r-\boldsymbol{x}^{\prime}\cdot\hat{\boldsymbol{x}},~~~~ \hat{\boldsymbol{x}}\equiv\frac{x}{r}$$
