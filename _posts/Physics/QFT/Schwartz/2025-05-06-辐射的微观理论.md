...
last_modified_at: 2025-05-10 17:05:52 +0800
...
$$\omega_n=\frac{2\pi}{L}\lvert \vec{n}\rvert c$$
...
于是, 这个公式代表的紫外灾难 (ultraviolet catastrophe) 是不可避免的: !*Pasted image 20250510140201.png*
这种理论和观测的不同启发普朗克假定每个电磁模式的能量是**量子化**的: 

$$E_n=\hbar\omega_n=\frac{2\pi}{L}\hbar\lvert \vec{n}\rvert =\lvert \vec{p}_n\rvert ,$$
...
$$m_n^2=E_n^2-\lvert \vec{p}_n\rvert ^2=0.$$
...
$$\begin{aligned}E(\omega)&=\int^\omega d^3\vec{n}\frac{\hbar\omega_n}{e^{\hbar\omega_n\beta}-1}=\int_{-1}^1d\cos\theta\int_0^{2\pi}d\phi\int_0^\omega d\lvert \vec{n}\rvert \frac{\lvert \vec{n}\rvert ^2\hbar\omega_n}{e^{\hbar\omega_n\beta}-1}\\&=4\pi\hbar\frac{L^{3}}{8\pi^{3}}\int_{0}^{\omega}d\omega^{\prime}\frac{\omega^{\prime3}}{e^{\hbar\omega^{\prime}\beta}-1}.\end{aligned}$$
...
$$a^{\dagger}a\ket{n} =\hat{N}\lvert n\rangle=n\rvert  n\rangle.$$

- 

$$\hat{N}a^\dagger\lvert n\rangle=a^\dagger aa^\dagger\rvert  n\rangle=a^\dagger\lvert n\rangle+a^\dagger a^\dagger a\rvert n\rangle=(n+1)a^\dagger\lvert n\rangle.$$
...
$$\Gamma\sim\lvert \mathcal{M}\rvert ^2\delta(E_f-E_i),$$

 其中 $$\mathcal{M}$$ 是初末态在相互作用哈密顿量上的投影: 

$$\mathcal{M}=\langle f\lvert H_{\mathrm{int}}\rvert i\rangle.$$
...
$$\begin{aligned}\mathcal{M}_{2\to1}&=\langle\mathrm{atom}_1;n_\omega+1\lvert (H_I^\dagger a^\dagger+H_Ia)\rvert \mathrm{atom}_2;n_\omega\rangle\\&=\langle\mathrm{atom}_1\lvert H_I^\dagger\rvert \mathrm{atom}_2\rangle\langle n_\omega+1\lvert a^\dagger\rvert n_\omega\rangle+\langle\mathrm{atom}_1\lvert H_I\rvert \mathrm{atom}_2\rangle\langle n_\omega+1\lvert a\rvert n_\omega\rangle\\&=\mathcal{M}_0^\dagger\langle n_\omega+1\lvert n_\omega+1\rangle\sqrt{n_\omega+1}+0\\&=\mathcal{M}_0^\dagger\sqrt{n_\omega+1}\end{aligned}$$

 其中 $$\mathcal{M}_0^\dagger=\langle\mathrm{atom}_1\lvert H_I^\dagger\rvert \mathrm{atom}_2\rangle$$, 于是有: 

$$\lvert \mathcal{M}_{2\to1}\rvert ^2=\lvert \mathcal{M}_0\rvert ^2(n_\omega+1).$$

我们可以类似地考虑一个 $$1\to2$$ 的过程, 不难得到: 

$$\begin{aligned}\mathcal{M}_{1\to2}&=\langle\mathrm{atom}_2;n_\omega-1\lvert H_I^\dagger a^\dagger+H_Ia\rvert \mathrm{atom}_1;n_\omega\rangle\\&=\langle\mathrm{atom}_2\lvert H_I\rvert \mathrm{atom}_1\rangle\langle n_\omega-1\lvert a\rvert n_\omega\rangle\\&=\mathcal{M}_0\sqrt{n_\omega}\end{aligned}$$

注意这里我们并没有把 $$2\to 1$$ 的跃迁概率拆成两个系数, 它直接就被 $$\mathcal{M}$$ 计算出来了, 于是我们能写出 $$\mathrm{d}n$$ 的方程: 

$$dn_2=-dn_1=-\lvert \mathcal{M}_{2\to1}\rvert ^2n_2+\lvert \mathcal{M}_{1\to2}\rvert ^2n_1=-\lvert \mathcal{M}_0\rvert ^2(n_\omega+1)n_2+\lvert \mathcal{M}_0\rvert ^2(n_\omega)n_1$$
...
其中模式数 $$d^{3}n$$ 被能量量子化条件 $$\Delta=\hbar \frac{2\pi}{L}\lvert \vec{n}\rvert $$ 转化为了对能量的积分（这个积分不用积出来, 下一步就求导回去了）. 
乘上偏振因子 $$2$$, 获得光强（能量密度谱）: 
...
$$dn_2=-dn_1=-\lvert \mathcal{M}_0\rvert ^2\left[\underbracket{ 1 }_{ A }+\underbracket{ \frac{\pi^2}{\hbar\omega^3} }_{ B }I(\omega)\right]n_2+\lvert \mathcal{M}_0\rvert ^2\left[\underbracket{ \frac{\pi^2}{\hbar\omega^3} }_{ B' }I(\omega)\right]n_1$$
