...
$$\omega_n=\frac{2\pi}{L}\left| \vec{n}\right|  c$$
...
$$E_n=\hbar\omega_n=\frac{2\pi}{L}\hbar\left| \vec{n}\right|  =\left| \vec{p}_n\right|  ,$$
...
$$m_n^2=E_n^2-\left| \vec{p}_n\right|  ^2=0.$$
...
$$\begin{aligned}E(\omega)&=\int^\omega d^3\vec{n}\frac{\hbar\omega_n}{e^{\hbar\omega_n\beta}-1}=\int_{-1}^1d\cos\theta\int_0^{2\pi}d\phi\int_0^\omega d\left| \vec{n}\right|  \frac{\left| \vec{n}\right|  ^2\hbar\omega_n}{e^{\hbar\omega_n\beta}-1}\\&=4\pi\hbar\frac{L^{3}}{8\pi^{3}}\int_{0}^{\omega}d\omega^{\prime}\frac{\omega^{\prime3}}{e^{\hbar\omega^{\prime}\beta}-1}.\end{aligned}$$
...
$$a^{\dagger}a\ket{n} =\hat{N}\left| n\rangle=n\right|   n\rangle.$$

- 

$$\hat{N}a^\dagger\left| n\rangle=a^\dagger aa^\dagger\right|   n\rangle=a^\dagger\left| n\rangle+a^\dagger a^\dagger a\right|  n\rangle=(n+1)a^\dagger\left| n\rangle.$$
...
$$\Gamma\sim\left| \mathcal{M}\right|  ^2\delta(E_f-E_i),$$

 其中 $$\mathcal{M}$$ 是初末态在相互作用哈密顿量上的投影: 

$$\mathcal{M}=\langle f\left| H_{\mathrm{int}}\right|  i\rangle.$$
...
$$\begin{aligned}\mathcal{M}_{2\to1}&=\langle\mathrm{atom}_1;n_\omega+1\left| (H_I^\dagger a^\dagger+H_Ia)\right|  \mathrm{atom}_2;n_\omega\rangle\\&=\langle\mathrm{atom}_1\left| H_I^\dagger\right|  \mathrm{atom}_2\rangle\langle n_\omega+1\left| a^\dagger\right|  n_\omega\rangle+\langle\mathrm{atom}_1\left| H_I\right|  \mathrm{atom}_2\rangle\langle n_\omega+1\left| a\right|  n_\omega\rangle\\&=\mathcal{M}_0^\dagger\langle n_\omega+1\left| n_\omega+1\rangle\sqrt{n_\omega+1}+0\\&=\mathcal{M}_0^\dagger\sqrt{n_\omega+1}\end{aligned}$$

 其中 $$\mathcal{M}_0^\dagger=\langle\mathrm{atom}_1\left| H_I^\dagger\right|  \mathrm{atom}_2\rangle$$, 于是有: 

$$\left| \mathcal{M}_{2\to1}\right|  ^2=\left| \mathcal{M}_0\right|  ^2(n_\omega+1).$$

我们可以类似地考虑一个 $$1\to2$$ 的过程, 不难得到: 

$$\begin{aligned}\mathcal{M}_{1\to2}&=\langle\mathrm{atom}_2;n_\omega-1\left| H_I^\dagger a^\dagger+H_Ia\right|  \mathrm{atom}_1;n_\omega\rangle\\&=\langle\mathrm{atom}_2\left| H_I\right|  \mathrm{atom}_1\rangle\langle n_\omega-1\left| a\right|  n_\omega\rangle\\&=\mathcal{M}_0\sqrt{n_\omega}\end{aligned}$$

注意这里我们并没有把 $$2\to 1$$ 的跃迁概率拆成两个系数, 它直接就被 $$\mathcal{M}$$ 计算出来了, 于是我们能写出 $$\mathrm{d}n$$ 的方程: 

$$dn_2=-dn_1=-\left| \mathcal{M}_{2\to1}\right|  ^2n_2+\left| \mathcal{M}_{1\to2}\right|  ^2n_1=-\left| \mathcal{M}_0\right|  ^2(n_\omega+1)n_2+\left| \mathcal{M}_0\right|  ^2(n_\omega)n_1$$
...
其中模式数 $$d^{3}n$$ 被能量量子化条件 $$\Delta=\hbar \frac{2\pi}{L}\left| \vec{n}\right|  $$ 转化为了对能量的积分（这个积分不用积出来, 下一步就求导回去了）. 
乘上偏振因子 $$2$$, 获得光强（能量密度谱）: 
...
$$dn_2=-dn_1=-\left| \mathcal{M}_0\right|  ^2\left[\underbracket{ 1 }_{ A }+\underbracket{ \frac{\pi^2}{\hbar\omega^3} }_{ B }I(\omega)\right]n_2+\left| \mathcal{M}_0\right|  ^2\left[\underbracket{ \frac{\pi^2}{\hbar\omega^3} }_{ B' }I(\omega)\right]n_1$$
